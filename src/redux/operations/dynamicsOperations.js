...
export const getCustomerSavingsForChart = createAsyncThunk(
  'dynamics/getCustomerSavingsForChart',
...
export const getCustomerSavingsForStatistic = createAsyncThunk(
  'dynamics/getCustomerSavingsForStatistic',
  async (periodData, { getState, rejectWithValue }) => {
    const { token } = getState().authorized;
    axiosHeaderToken.set(token);
    try {
      const savingsDataForStats = await getCustomerSavingsForStatisticApi(
        periodData
      );
