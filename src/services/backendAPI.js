...
export const getLimitsAndTotalsApi = () => {
    return axios
        ...
        .then(({ data: { monthLimit, dailyLimit, totalByMounth, totalByDay } }) => ({
            monthLimit,
            dailyLimit,
            totalByMounth,
            totalByDay
...
export const getCustomerSavingsForChartApi = () => {
    return axios
        .get('/api/dynamics')
        .then(({ data: {
            statByYear, year, month, accumulatedProc,
            accumulatedUah, squareМeters, accumToOneMoreMeters } }) => ({
                statByYear, year, month, accumulatedProc,
                accumulatedUah, squareМeters, accumToOneMoreMeters
            }));
}
...
export const getCustomerSavingsForStatisticApi = ({ year, month }) => {
    return axios
        .get('/api/dynamics/by-month', { year, month })
        .then(({ data: { income, expense, accumulated, plan, planInProcent } }) => ({
            income, expense, accumulated, plan, planInProcent
        }));
}
